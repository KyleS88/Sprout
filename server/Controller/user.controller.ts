...
export const register = async (req: Request, res: Response) => {
    try {
        ...
        const user: QueryResultRow = await pool.query("SELECT * FROM connector.users WHERE email = $1 OR username = $2;", [email, username, ]);
        ...
        const query: string = `INSERT INTO connector.users (username, email, password, user_id) VALUES ($1, $2, $3, $4);`
...
export const login = async (req: Request, res: Response) => {
    try {   
        ...
        const user = await pool.query("SELECT * FROM connector.users WHERE email=$1;", [email,]);
        if (user.rows.length === 0) {
            return res.status(400).json({
